# Optimized Attention On Ascend NPU
...
- **自适应序列长度策略**：为短、中、长序列设计了不同的 UB 内存布局和计算流程，确保在任何场景下都能高效利用片上缓存。
- **混合精度**：支持使用 `bfloat16` 进行计算和存储，但在需要高精度的累加步骤中使用 `float32`，实现了性能和准确性的平衡。
