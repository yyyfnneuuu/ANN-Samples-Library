// HnswSearchLayer将被彻底改造，以利用FastScan和无重排特性。
...
// 获取c的邻居列表 (这步仍然是随机I/O，读取一个图页面)
HnswLoadUnvisitedNeighbors(...);

// 核心改造点: 批量处理邻居
int unvisited_len = ...;
HnswElement* unvisited = ...;

// 1. 将一批unvisited邻居的RabitqItemData所在页面预取(prefetch)到内存
//    因为RabitqItemData是连续存储的，这里的I/O模式更友好
